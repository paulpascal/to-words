  ...
  _createClass(ToWords, [{
    ...
    value: function convert(number) {
      ...
      if (ns.includes('.')) {
        ...
        return this.convertInternal(split[0], options) + (options.currency ? ' ' + options.currencyLabel + ' And ' : ' Point ') + this.convertInternal(split[1], options) + (options.currency ? ' Only' : '');
      }
